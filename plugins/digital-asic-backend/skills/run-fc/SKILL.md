...
1. Verify RTL files exist in `$PROJECT_PATH/hdl/` (read `fc/scripts/stages/init.tcl` to see which files are needed).
...
3. Check if `fc/${DESIGN_NAME}.dlib` already exists:
   - If yes, **warn the user** and create a backup:
     ...
     ```
4. Ensure `fc/scripts/flows/run_full.tcl` exists.
