...
1. Verify RTL files exist in `$PROJECT_PATH/hdl/` (read `fc/scripts/design_setup.tcl` to see which files are needed).
2. Verify PDK path is accessible (check `LIBRARY_PATH` from `fc_setup.tcl`).
3. Check if `fc/${DESIGN_NAME}.dlib` already exists:
   - If yes, **warn the user** and create a backup:
     ...
     ```
4. Ensure `fc/scripts/fc_run_full.tcl` exists.
...
```bash
cd $PROJECT_PATH/fc/temp && nohup fc_shell -f ../scripts/fc_run_full.tcl > ../report/run_fc.log 2>&1 &
echo $! > /tmp/.fc_run_pid
...
```bash
cd $PROJECT_PATH/fc/temp && fc_shell -f ../scripts/fc_run_full.tcl 2>&1 | tee ../report/run_fc.log
```
