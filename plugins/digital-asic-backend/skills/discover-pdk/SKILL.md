...
> **NDM libraries are directories**, not files. A `.ndm` library such as `stdcell.ndm` is a directory containing internal files (e.g. `reflib.ndm`). Always search with `-type d` to find the library root — never descend into it and pick an internal file.
